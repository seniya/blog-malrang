...
export function PostForm({ post }: { post?: Post }) {
  ...
  return <form onSubmit={submit} className="space-y-5">
    ...
    <div><label className="mb-1 block">본문 (Markdown)</label><Controller name="content" control={control} render={({ field }) => <MarkdownEditor {...field} />} />{errors.content && <small className="text-red-600">{errors.content.message}</small>}</div>
