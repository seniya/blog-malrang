...
import { getPostTaxonomyNames, getPublishedTagBySlug, listPublishedPostsByTag } from "@/features/posts/repository";
...
export default async function TagPage({ params }: Props) {
  ...
  const tag = getPublishedTagBySlug(db, slug);
  if (!tag) notFound();
  const posts = listPublishedPostsByTag(db, slug).map((post) => ({ ...post, ...getPostTaxonomyNames(db, post.id) }));
